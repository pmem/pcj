...
public class PersistentConcurrentLinkedQueue <E extends PersistentObject> extends PersistentObject implements Iterable<E> {	
	...
	public static final ObjectType<PersistentConcurrentLinkedQueue> TYPE = ObjectType.fromFields(PersistentConcurrentLinkedQueue.class, HEAD, TAIL);

	public PersistentConcurrentLinkedQueue() {
		super(TYPE);
		PersistentNode<E> sentinel = new PersistentNode<>();
		setHeadAndTail(sentinel, sentinel);
	}

	public PersistentConcurrentLinkedQueue(Collection<? extends E> c) {
		super(TYPE);
	...
	public static class PersistentNode<E extends PersistentObject> extends PersistentObject {
		private static final ObjectField<PersistentObject> ITEM = new ObjectField<>();
		...
		public PersistentNode(E item) {
			this(item, new PersistentAtomicReference<PersistentNode<E>>());
		}

		public PersistentNode() {
			this(null, new PersistentAtomicReference<PersistentNode<E>>());
		}
		...
		public synchronized E getItem() {
			return (E)getObjectField(ITEM);
		}

		public synchronized void setItem(E item) {
			setObjectField(ITEM, item);
		}

		@SuppressWarnings("unchecked")
		public synchronized PersistentNode<E> getNext() {
			return (PersistentNode<E>)getObjectField(NEXT).get();
		}

		public synchronized void setNext(PersistentAtomicReference<PersistentNode<E>> next) {
			setObjectField(NEXT, next);
		}
		
		public synchronized void setNextNode(PersistentNode<E> node) {
		...
		public synchronized boolean casItem(E expect, E update) {
			return compareAndSetItem(expect, update);
		}

		public synchronized boolean compareAndSetItem(E expect, E update) {
			if(getItem() != expect) return false;
			setItem(update);
			return true;
		}
	}
