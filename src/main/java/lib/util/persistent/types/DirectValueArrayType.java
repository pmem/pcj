...
public class DirectValueArrayType<T extends AnyPersistent> extends ArrayType<T> { 
    public static final long LENGTH_OFFSET = 0;
    public static final long ELEMENTS_OFFSET = LENGTH_OFFSET + Types.INT.size();

    public DirectValueArrayType(Class<T> cls, PersistentType elementType) {
        super(cls, ObjectType.Kind.Reference, elementType);
    }

    public DirectValueArrayType(Class<T> cls, ObjectType.Kind kind, PersistentType elementType) {
        super(cls, kind, elementType);
    }
    ...
    public String toString() {
        return "DirectValueArrayType(" + name() + ", " + elementType() + ")";
    }
