...
public class PersistentSkipListMap2<K extends AnyPersistent, V extends AnyPersistent> extends PersistentObject implements ConcurrentNavigableMap<K,V>, PersistentSortedMap<K,V> {
    ...
    static {
        ...
        if (statics == null)
            ObjectDirectory.put("PersistentSkipListMap2_statics", statics = new Statics());
    }
    ...
    Node<K,V> headnode() { return getObjectField(HEADNODE); }

    void headnode(Node<K,V> headnode) { setObjectField(HEADNODE, headnode); }

    public PersistentArrayList  getMetaList() { return getObjectField(META_LIST); }

    private void setMetaList(PersistentArrayList meta) { setObjectField(META_LIST, meta); }
    ...
    private void initialize() {
        ...
        values = null;
        descendingMap = null;
        headnode(new Node<K,V>(null, baseHeader(), null));
    ...
    public synchronized void rebuild(){
        ...
        int max = getIntField(LEVEL);
        System.out.println("max level is "+max);
        int count=0;
        try{
        for(Node<K,V> z = headnode().next().next().next().next(); z != null; count++) {
            if (z.value() == null) {z = z.next(); continue;}
        ...
        //    if ((rnd & 0x80000001) == 0) { // test highest and lowest bits
                ...
                if (level <= (h.level)) {
                    ...
                    level = Math.min(level, max); // hold in array and later pick the one to use
                    //level = max+1;
                ...
                for (Index<K,V> q = h, r = q.right, t = idx;;) {
                    if (q == null || t == null)
                        break splice;
                    if (r != null && !r.node.is(t.node)) {
                        /*Node<K,V> n = r.node;
                        int c = cpr(cmp, z.key(), n.key());
                        /*if (n.value() == null) {
                            if (!q.unlink(r))
                                break;
                            r = q.right;
        ...
        z = z.next().next().next().next();
        }}catch (NullPointerException e){}
        //rebuild = false;
        long end = System.currentTimeMillis();
        System.out.println("Rebuilding took "+(end-start)+" for "+count+" nodes");
        System.out.println("head nodes is now at level "+head.level);
    }
    ...
    /* ---------------- Indexing -------------- */
    static class Index<K extends AnyPersistent,V extends AnyPersistent> {
        final Node<K,V> node;
     ...
     */
    static final class HeadIndex<K extends AnyPersistent,V extends AnyPersistent> extends Index<K,V> {
        final int level;
    ...
    /**
     * Returns a shallow copy of this {@code PersistentSkipListMap2}
     * instance. (The keys and values themselves are not cloned.)
     *
     * @return a shallow copy of this map
     */
   /* public PersistentSkipListMap2<K,V> clone() {
        try {
            @SuppressWarnings("unchecked")
            PersistentSkipListMap2<K,V> clone =
                (PersistentSkipListMap2<K,V>) super.clone();
            clone.initialize();
    ...
    public boolean equals(AnyPersistent o) {
        if (o == this)
            return true;
        if (!(o instanceof PersistentSkipListMap2))
            return false;
        PersistentSkipListMap2<?,?> m = (PersistentSkipListMap2<?,?>) o;
        try {
    ...
    abstract class Iter<T> implements Iterator<T> {
        ...
        public void remove() {
            ...
            PersistentSkipListMap2.this.remove(l.key());
            lastReturned = null;
        }
    ...
    static final class KeySet<E extends AnyPersistent>
        ...
        public Iterator<E> iterator() {
            if (m instanceof PersistentSkipListMap2)
                return ((PersistentSkipListMap2<E,AnyPersistent>)m).keyIterator();
            else
                return ((PersistentSkipListMap2.SubMap<E,AnyPersistent>)m).keyIterator();
        }
        ...
        public Spliterator<E> spliterator() {
            if (m instanceof PersistentSkipListMap2)
                return ((PersistentSkipListMap2<E,?>)m).keySpliterator();
            else
    ...
    static final class Values<E extends AnyPersistent> extends AbstractCollection<E> {
        ...
        public Iterator<E> iterator() {
            if (m instanceof PersistentSkipListMap2)
                return ((PersistentSkipListMap2<?,E>)m).valueIterator();
            else
        ...
        public Spliterator<E> spliterator() {
            if (m instanceof PersistentSkipListMap2)
                return ((PersistentSkipListMap2<?,E>)m).valueSpliterator();
            else
    ...
    static final class EntrySet<K1 extends AnyPersistent,V1 extends AnyPersistent> extends AbstractSet<Map.Entry<K1,V1>> {
        ...
        public Iterator<Map.Entry<K1,V1>> iterator() {
            if (m instanceof PersistentSkipListMap2)
                return ((PersistentSkipListMap2<K1,V1>)m).entryIterator();
            else
        ...
        public Spliterator<Map.Entry<K1,V1>> spliterator() {
            if (m instanceof PersistentSkipListMap2)
                return ((PersistentSkipListMap2<K1,V1>)m).entrySpliterator();
            else
    ...
    /** Underlying map */
        private final PersistentSkipListMap2<K,V> m;
        ...
        SubMap(PersistentSkipListMap2<K,V> map,
        ...
        boolean isBeforeEnd(PersistentSkipListMap2.Node<K,V> n,
        ...
        PersistentSkipListMap2.Node<K,V> loNode(Comparator<? super K> cmp) {
            if (lo == null)
                return m.findFirst();
            else if (loInclusive)
        ...
        PersistentSkipListMap2.Node<K,V> hiNode(Comparator<? super K> cmp) {
            if (hi == null)
                return m.findLast();
            else if (hiInclusive)
        ...
        K lowestKey() {
            ...
            PersistentSkipListMap2.Node<K,V> n = loNode(cmp);
        ...
        K highestKey() {
            Comparator<? super K> cmp = m.comparator;
            PersistentSkipListMap2.Node<K,V> n = hiNode(cmp);
            if (n != null) {
                K last = n.key();
        ...
        public int size() {
            Comparator<? super K> cmp = m.comparator;
            long count = 0;
            for (PersistentSkipListMap2.Node<K,V> n = loNode(cmp);
                 isBeforeEnd(n, cmp);
                 n = n.next()) {
                if (n.getValidValue() != null)
                    ++count;
    ...
    static {
        try {
            ...
            f.setAccessible(true);
            UNSAFE = (Unsafe)f.get(null);
            Class<?> k = PersistentSkipListMap2.class;
