...
public class TestCases {
	public static void main(String[] args) {
		...
		ba.set(50, (byte)10);
		byte b = ba.get(50);
		System.out.println("ba.get(50), should be 10 -> " + b);
		ba = null; System.gc();

		System.out.println("new PersistentString1");
		PersistentString sx1 = new PersistentString("xxxxxx");
		System.out.println("created PersistentStringX " + sx1 + ", len = " + sx1.length());
		sx1 = null;
		System.gc();
		...
		System.out.println("Tested init of FloatArray");

		System.out.println("new PersistentString2");
		PersistentString sx2 = new PersistentString("xxxxxx");
		System.out.println("created PersistentStringX " + sx2 + ", len = " + sx2.length());

		PersistentDoubleArray da = new PersistentDoubleArray(100);
		da.set(50, 12.5d);
		double d = da.get(50);
		System.out.println("da.get(50), should be 12.5 -> " + d);

		System.out.println("new PersistentLongArray(100)");
		PersistentLongArray la = new PersistentLongArray(100);
		la.set(50, 10);
		long l = la.get(50);
		System.out.println("la.get(50), should be 10 -> " + l);

		System.out.println("new PersistentImmutableLongArray(100)");
		PersistentImmutableLongArray ila = new PersistentImmutableLongArray(new long[] {10, 20, 30, 40, 50, 60, 70});
		long il = ila.get(4);
		System.out.println("ila.get(4), should be 50 -> " + il);

		System.out.println("new PersistentString");
		PersistentString s = new PersistentString("hello");
		System.out.println("created PersistentString " + s + ", len = " + s.length());
		...
		System.out.println("mx.p() = " + mx.p());
		System.out.println("mx.s() = " + mx.s());
		System.out.println("mx.x() = " + mx.x());
		...
		System.out.println("pl.toLong() = " + pl.longValue());

		Mixed m1 = new Mixed(new PersistentString("dog"), 13);
		PersistentTuple2<Mixed, PersistentLong> t1 = new PersistentTuple2<>(m, pl);
		System.out.println("t1._1() = " + t1._1());
		System.out.println("t1._2() = " + t1._2());
		System.out.println("t1._1().s() = " + t1._1().s());
		System.out.println("t1._1().x() = " + t1._1().x());
		System.out.println("t1._2().toLong() = " + t1._2().longValue());

		PersistentDouble pd = new PersistentDouble(123.45d);
		System.out.println("pd.toDouble() = " + pd.doubleValue());

		PersistentArray<PersistentObject> oa = new PersistentArray<>(m, t1);
		System.out.println("oa.get(0) = " + oa.get(0));
		System.out.println("oa.get(1) = " + oa.get(1));

		PersistentArray<Mixed> ma = new PersistentArray<>(m, m);
		System.out.println("ma.get(0) = " + ma.get(0));
		System.out.println("ma.get(1) = " + ma.get(1));

		PersistentArray<PersistentArray<Mixed>> maa = new PersistentArray<PersistentArray<Mixed>>(ma, ma);
		System.out.println("maa.getClass(): "+maa.getClass());
		System.out.println("maa.get(0) = " + maa.get(0));
		System.out.println("maa.get(1) = " + maa.get(1));

		ExtendedMix em = new ExtendedMix(s1, 100, s2, 200);
		System.out.println("em.s() = " + em.s());
		System.out.println("em.x() = " + em.x());
		System.out.println("em.es() = " + em.es());
		System.out.println("em.ei() = " + em.ei());
		...
		System.out.println("tup3 contents: " + tup3._1() + ", " + tup3._2() + ", " + tup3._3());

		PersistentImmutableTuple2<Employee1, Point> tup2 = new PersistentImmutableTuple2<>(bob1, px);
		System.out.println("tup2 contents: " + tup2._1() + ", " + tup2._2());
		...
		for (int i=0; i<parr.length(); i++){
			try{
				System.out.println("parr1: Element at index "+i+" is "+parr.get(i).toString());
			} catch (NullPointerException e){
				System.out.println("parr1: Element at index "+i+" is NULL");
			}
		}

		for (int i=0; i<parr2.length(); i++){
			try{
				System.out.println("parr2: Element at index "+i+" is "+parr2.get(i).toString());
			} catch (NullPointerException e){
				System.out.println("parr2: Element at index "+i+" is NULL");
			}
		}
		...
		System.out.println("InnerStatic.getId() = " + InnerStatic.getId());

		//ValueType experiment
		Long256 l256 = new Long256(298374, 349857, 23094, 2983749);
		System.out.println(l256);
		l256 = null;
		System.gc();
		...
		System.out.println("****************Testing PersistentSkipListMap ****************");
		...
		System.out.println("****************Testing PersistentLinkedQueue ****************");
		...
		if(plqPmem == null) {
			...
			System.out.println("PersistentLinkedQueue doesn't exist...saving to pmem");
			System.out.println(plq2);
		}
		else{
			System.out.println("Retrieving PersistentLinkedQueue from pmem");
			System.out.println(plqPmem);
